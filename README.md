...
Release binaries include the version tag (e.g. `rc -v` → `rc version v0.7.5`). Update in-place with `rc --update` or via the web UI menu. The update process verifies the new binary before restarting, and recovers automatically if the new process fails to start.
